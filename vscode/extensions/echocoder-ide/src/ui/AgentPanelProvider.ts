...
import { AgentEvent, isTextDelta, isToolCall, isToolResult, isResultSuccess, isResultError } from '../types/agent-events';

export class AgentPanelProvider implements vscode.WebviewViewProvider {
  public static readonly viewType = 'echocoder.agentPanel';
  ...
  private async handleWebviewMessage(message: any): Promise<void> {
    switch (message.type) {
      case 'sendPrompt': {
        ...
        try {
          await this.processManager.spawn({ prompt, cwd });
        } catch (err) {
          this.postMessage({
            type: 'agentError',
