...
export class Planner {
  ...
  public async generatePlan(
    ...
    const assembled = await this.promptAssembler.assembleChatPrompt(
          ...
          if (!resolved) {
            ...
            try {
              ...
              this.outputChannel.appendLine(
                `[Planner] Plan generated: ${plan.steps.length} steps, risk=${plan.risk}`
              );
              resolve(plan);
            } catch (err) {
   ...
   */
  private parseAgentResponse(rawText: string, userPrompt: string): Plan {
    ...
    const steps: PlanStep[] = parsed.steps.map((step: any, i: number) => ({
   ...
   */
  private buildFallbackPlan(rawText: string, userPrompt: string): Plan {
  ...
  private normalizeCategory(raw: unknown): PlanStep['category'] {
    const valid = ['read', 'write', 'create', 'delete', 'execute', 'refactor', 'test', 'other'];
    return typeof raw === 'string' && valid.includes(raw)
      ? (raw as PlanStep['category'])
      : 'other';
  }

  private normalizeRisk(raw: unknown): 'low' | 'medium' | 'high' {
