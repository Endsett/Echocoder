...
<body>
  <div class="empty-state">${message}</div>
    ...
    const stepsHtml = steps.map(s => {
      ...
      if (s.status === 'pending') iconClass = 'codicon-clock';
      else if (s.status === 'running') iconClass = 'codicon-sync codicon-modifier-spin';
      else if (s.status === 'done') { iconClass = 'codicon-check'; completedSteps++; }
      else if (s.status === 'failed') iconClass = 'codicon-error';
      else if (s.status === 'skipped') { iconClass = 'codicon-debug-step-over'; completedSteps++; }
